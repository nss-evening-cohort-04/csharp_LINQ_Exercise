...
namespace LINQ_Practice
{
    ...
    public class LINQ_Practice_MethodChaining
    {
        ...
        {
            var ActualCohorts = PracticeData.Where(cohort => cohort.PrimaryInstructor.FirstName == "Zachary" || cohort.JuniorInstructors.Any(instructor => instructor.FirstName == "Zachary")).ToList();
        ...
        {
            var ActualCohorts = PracticeData.Where(cohort => cohort.FullTime == false && cohort.PrimaryInstructor.Active == true && cohort.JuniorInstructors.All(instructor => instructor.Active == true)).ToList();
        ...
        {
            var ActualCohorts = PracticeData.Where(cohort => cohort.Students.Any(student => student.FirstName == "Kate") || cohort.PrimaryInstructor.FirstName == "Kate" || cohort.JuniorInstructors.Any(instructor => instructor.FirstName == "Kate")).ToList();
        ...
        {
            var student = PracticeData.SelectMany(cohort => cohort.Students).OrderBy(x =>x.Birthday).First();
        ...
        {
            var student = PracticeData.SelectMany(cohort => cohort.Students).OrderBy(x => x.Birthday).Last();
        ...
        {
            var ActualStudents = PracticeData.SelectMany(cohort => cohort.Students).Where(x =>x.Active == false).ToList();
